...
func (ts *PostgresTestSuite) TestOrderSetStatusBatch() {
	...
	err = ts.storage.OrderSetStatusBatch(ctx, []models.OrderID{orderData[1].ID, orderData[3].ID}, models.OrderStatusProcessing)
	ts.NoError(err)
	...
	err = ts.storage.OrderSetStatusBatch(ctx, []models.OrderID{orderData[0].ID, orderData[2].ID}, models.OrderStatusProcessed)
	ts.NoError(err)
