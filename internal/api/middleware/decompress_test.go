...
func (s *DecompressSuite) deflateCompress(data []byte) []byte {
	var b bytes.Buffer

	w, err := flate.NewWriter(&b, gzip.BestCompression)
	s.Require().NoError(err)
...
func (s *DecompressSuite) TestGzipDecompress() {
	testCases := []struct {
		name     string
		encoding string
		msg      []byte
...
func (s *DecompressSuite) TestDeflateDecompress() {
	testCases := []struct {
		name     string
		encoding string
		msg      []byte
		...
		},
		{
			name:     "Positive: gzip encoding",
			encoding: "deflate",
			msg:      s.deflateCompress([]byte(`{"message":"Hello"}`)),
			want:     []byte(`{"message":"Hello"}`),
		},
		{
			name:     "Positive: wrong content encoding",
